  ...
  public:
    ...
    ALPAKA_FN_ACC void operator()(TAcc const &acc,
      ...
      for (auto idx : cms::alpakatools::uniform_elements(acc, *nCells)) {
        ...
        unsigned int  maxNl = 0;

        // find maxNl
        for (auto it : thisCell.tracks()) {
          auto nl = tracks_view.hitIndices().size(it);//tracks_view[it].nLayers();
        ...
        for (auto it : thisCell.tracks()) {
          if (tracks_view.hitIndices().size(it) < maxNl)//tracks_view[it].nLayers() < maxNl)
    ...
    ALPAKA_FN_ACC void operator()(TAcc const &acc,
      ...
      for (auto idx : cms::alpakatools::uniform_elements(acc, hitToTuple.nOnes())) {
        ...
        for (auto ip = hitToTuple.begin(idx); ip < hitToTuple.end(idx) - 1; ++ip) {
          ...
          auto cti = tracks_view[it].state()(3);
          auto e2cti = tracks_view[it].covariance()(12);
          auto nli = TracksUtilities<TrackerTraits>::nHits(tracks_view, it);//tracks_view[it].nLayers();
          for (auto jp = ip + 1; jp < hitToTuple.end(idx); ++jp) {
            ...
            if ((opi - opj) * (opi - opj) > dop)
              continue;
            auto nlj = TracksUtilities<TrackerTraits>::nHits(tracks_view, jt);//tracks_view[jt].nLayers();
    ...
    ALPAKA_FN_ACC void operator()(TAcc const &acc,
      ...
      for (auto idx : cms::alpakatools::uniform_elements(acc, hitToTuple.nOnes())) {
        ...
        for (auto it = hitToTuple.begin(idx); it != hitToTuple.end(idx); ++it) {
          ...
          auto nl = TracksUtilities<TrackerTraits>::nHits(tracks_view, *it);//tracks_view[*it].nLayers();
          maxNl = std::max(nl, maxNl);
        }
